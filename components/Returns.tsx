...
import { Search, RotateCcw, AlertCircle, ArrowRight, History, Calendar, Clock as ClockIcon, Trash2, AlertTriangle, Copy, Check } from 'lucide-react';
import { Invoice, ReturnRecord, ReturnItem } from '../types';
...
const Returns: React.FC<ReturnsProps> = ({ invoices, returns, onAddReturn, onDeleteReturn, onRestockItem, onShowToast }) => {
  ...
  const processReturn = () => {
    ...
    const now = new Date();
    const record: ReturnRecord = { id: `RET-${now.getTime().toString().slice(-6)}`, invoiceId: selectedInvoice.id, items, totalRefund: Number(totalRefund.toFixed(2)), date: now.toLocaleDateString('ar-EG'), time: now.toLocaleTimeString('ar-EG'), timestamp: now.getTime(), isDeleted: false };
    onAddReturn(record);
      ...
      {selectedInvoice && (
        <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl overflow-hidden animate-in">
          ...
          <div className="p-8 space-y-8">
            <div className="bg-amber-50 p-5 rounded-2xl border border-amber-100 text-[11px] text-amber-800 font-bold">يتم رد المبلغ بناءً على "السعر الفعلي" المسدد بالفاتورة (Weighted Refund).</div>
        ...
        <div className="overflow-x-auto"><table className="w-full text-right text-xs min-w-[700px]"><thead className="bg-slate-50 text-slate-400 font-black text-[8px] border-b"><tr><th className="px-8 py-5">رقم المرتجع</th><th className="px-8 py-5">الأصل</th><th className="px-8 py-5">التاريخ</th><th className="px-8 py-5 text-center">القيمة</th><th className="px-8 py-5"></th></tr></thead>
              <tbody className="divide-y">{activeReturns.length === 0 ? (<tr><td colSpan={5} className="p-16 text-center text-slate-300 font-bold italic">لا توجد مرتجعات نشطة</td></tr>) : activeReturns.map(rec => (
                       <tr key={rec.id} className="hover:bg-slate-50/50">
                          ...
                          <td className="px-8 py-5 text-left"><button onClick={() => setConfirmDelete({id: rec.id, reason: ''})} className="p-2 text-slate-300 hover:text-rose-600 transition-colors"><Trash2 size={16}/></button></td>
                       </tr>))}</tbody></table></div>
      </div>
