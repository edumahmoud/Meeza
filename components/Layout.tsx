...
const Layout: React.FC<LayoutProps> = ({ currentView, setView, products, onReset, onRestore, onProductClick, children, toast, onCloseToast }) => {
  ...
  const DeveloperSection = () => (
    <div className="mt-3 p-2.5 bg-slate-900 rounded-xl text-white overflow-hidden relative border border-slate-800">
      <div className="relative z-10">
        <p className="text-[7px] font-black text-indigo-400 uppercase tracking-widest mb-1 flex items-center gap-1">
          <Terminal size={8} /> منشئ النظام
        </p>
        <p className="text-[10px] font-black mb-1.5">ميزة POS - م. محمود</p>
        <div className="flex gap-2">
          <a href="https://www.facebook.com/m.helely0" target="_blank" className="p-1 bg-white/5 hover:bg-indigo-600 rounded transition-colors" title="Facebook"><Facebook size={10} /></a>
          <a href="https://wa.me/201503336555" target="_blank" className="p-1 bg-white/5 hover:bg-emerald-600 rounded transition-colors" title="WhatsApp"><MessageSquare size={10} /></a>
          <a href="#" className="p-1 bg-white/5 hover:bg-slate-600 rounded transition-colors" title="Website"><Globe size={10} /></a>
        </div>
      </div>
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden font-['Cairo']" dir="rtl">
      ...
      <aside className={`fixed lg:static inset-y-0 right-0 w-64 h-screen max-h-screen bg-white border-l border-slate-200 flex flex-col no-print shadow-xl z-[60] transform transition-transform duration-300 overflow-hidden ${isSidebarOpen ? 'translate-x-0' : 'translate-x-full lg:translate-x-0'}`}>
        <div className="p-5 flex items-center justify-between border-b border-slate-50 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div className="truncate">
              ...
              <p className="text-[9px] text-slate-400 font-bold uppercase mt-1 tracking-widest">إدارة ذكية</p>
            </div>
          </div>
        ...
        <div className="mt-auto p-4 bg-slate-50/50 border-t border-slate-100 shrink-0">
          <SystemButtons />
          <DeveloperSection />
        </div>
        ...
        <header className="h-20 min-h-[80px] bg-white border-b border-slate-200 flex items-center justify-between px-4 md:px-8 sticky top-0 z-40 no-print">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-lg md:text-xl font-black text-slate-800 tracking-tighter">
              {navItems.find(i => i.id === currentView)?.label || 'الرئيسية'}
            </h2>
          </div>
                            ...
                            <div className="flex-1 text-right">
                              ...
                              <div className="flex items-center gap-2 mt-0.5">
                                ...
                                <button 
                                  ...
                                  className={`text-[8px] font-black px-1.5 py-0.5 rounded flex items-center gap-1 transition-all ${copiedCode === item.code ? 'bg-emerald-500 text-white' : 'bg-slate-100 text-slate-400 hover:bg-indigo-600 hover:text-white'}`}
                                >
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[1000] flex items-center justify-center p-4">
          <div className="bg-white rounded-[2rem] w-full max-w-sm shadow-2xl overflow-hidden animate-in">
            <div className={`p-8 text-center ${confirmDialog.type === 'reset' ? 'bg-rose-50' : 'bg-indigo-50'}`}>
              <div className={`w-16 h-16 mx-auto mb-5 rounded-2xl flex items-center justify-center shadow-lg ${confirmDialog.type === 'reset' ? 'bg-rose-600 text-white' : 'bg-indigo-600 text-white'}`}>
                {confirmDialog.type === 'reset' ? <RefreshCcw size={32} /> : <ShieldCheck size={32} />}
              </div>
              <h3 className="text-xl font-black mb-2 text-slate-800">{confirmDialog.type === 'reset' ? 'تصفير نظام ميزة' : 'استعادة النسخة'}</h3>
              <p className="text-slate-500 text-xs font-bold leading-relaxed px-4">
                {confirmDialog.type === 'reset' ? 'هل أنت متأكد من تصفير كافة البيانات؟' : 'سيتم استبدال البيانات الحالية ببيانات النسخة الاحتياطية.'}
              </p>
            </div>
            <div className="p-5 bg-slate-50 flex gap-3 border-t border-slate-100">
              <button onClick={() => setConfirmDialog(null)} className="flex-1 py-2.5 bg-white border border-slate-200 text-slate-600 font-black rounded-xl text-[10px] hover:bg-slate-100 transition-all">تراجع</button>
              <button onClick={() => { if (confirmDialog.type === 'reset') onReset(); else { onRestore(confirmDialog.data); setConfirmDialog(null); } }} className={`flex-[1.5] py-2.5 text-white rounded-xl font-black text-[10px] shadow-xl transition-all ${confirmDialog.type === 'reset' ? 'bg-rose-600 hover:bg-rose-700' : 'bg-indigo-600 hover:bg-indigo-700'}`}>تأكيد العملية</button>
            </div>
          </div>
        </div>
      )}
