...
export const useInventory = () => {
  ...
  const updateStockWAC = (productId: string, addedQty: number, newPurchasePrice: number, updatedRetailPrice?: number, updatedDescription?: string) => {
    setProducts(prev => prev.map(p => {
      if (p.id !== productId) return p;
      
      const currentStock = p.stock || 0;
      const currentWPrice = p.wholesalePrice || 0;
      
      const totalCostOfCurrent = currentStock * currentWPrice;
      const totalCostOfNew = (addedQty || 0) * (newPurchasePrice || 0);
      const totalStock = currentStock + (addedQty || 0);
      ...
      return {
        ...
        retailPrice: updatedRetailPrice ?? p.retailPrice,
        description: updatedDescription ?? p.description
      };
    }));
  };
  ...
  const deductStock = (productId: string, qty: number) => {
    setProducts(prev => prev.map(p => 
      p.id === productId ? { ...p, stock: Math.max(0, p.stock - (qty || 0)) } : p
    ));
  };

  const restockItem = (productId: string, qty: number) => {
    setProducts(prev => prev.map(p => 
      p.id === productId ? { ...p, stock: p.stock + (qty || 0) } : p
    ));
  };
